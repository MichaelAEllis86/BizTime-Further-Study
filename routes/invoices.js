...
// allows paying of invoices accepts request body json of {amt, paid} and a route parameter for id
// if bill was paid, paid date and paid status are added
// if bill was unpaid, paid date goes back to null.
// if no paid "status/boolean" was given in request body json, we just update the amount and don't touch anything else.
// req.body formatting rules!----> if changing invoice amt only----> {'amt':150.25} if paying an unpaid bill ----> {amt:150.23,paid:true} if UNPAYING a PAID bill ----> {amt:150.23,paid:false}"
router.put("/pay/:id", middleware.isValidRequestBodyPayInvoice, async function updateAndPayInvoice(req,res,next){
    try{
        console.log("this is the request body---->", req.body)
        let id=req.params.id
        let paid=req.body.paid
        console.log("this is type of paid",typeof(paid))
        const invoiceDataQuery=await db.query(`SELECT id, comp_code, amt, paid, add_date, paid_date FROM invoices WHERE id=$1`,[id])
        console.log("this is the result of invoiceDataQuery  ---->" , invoiceDataQuery)
        console.log("this is the result.rows ---->" , invoiceDataQuery.rows)
        if (invoiceDataQuery.rows.length ===0){
            throw new ExpressError("invoice not found", 404)}
        let data=invoiceDataQuery.rows[0]
        if (data.paid === false && paid ===true ){
            let results=await db.query(`UPDATE invoices SET amt=$1, paid=$2, paid_date=CURRENT_DATE WHERE id=$3 RETURNING id, comp_code, amt, paid, add_date, paid_date`,[req.body.amt,true,id])
            console.log("this is the result ---->" , results)
            console.log("this is the result.rows ---->" , results.rows)
            return res.json({invoice:results.rows[0]})
        }
        if (data.paid === true && paid ===false ){
            let results=await db.query(`UPDATE invoices SET amt=$1, paid=$2, paid_date=null WHERE id=$3 RETURNING id, comp_code, amt, paid, add_date, paid_date`,[req.body.amt,false,id])
            console.log("this is the result ---->" , results)
            console.log("this is the result.rows ---->" , results.rows)
            return res.json({invoice:results.rows[0]})
        }
